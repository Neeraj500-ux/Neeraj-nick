...
export function Editor({
  ...
  const { data, save, user } = useWorkspace();
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(submit)}>
          ...
          {table === "employees" && (
            <label>
              ...
              <select {...register("role")}>
                {manageableRoles(user?.role || "employee").map((role) => (
                  <option value={role} key={role}>{role.replace("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase())}</option>
                ))}
              </select>
            </label>
          )}
        ...
        {table === "employees" && <p className="form-note">This adds a workspace directory record. Create Firebase Authentication accounts through a trusted administrator flow.</p>}
